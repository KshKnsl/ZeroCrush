...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        ...
        console.error(err);
        toast.error('Failed to fetch analytics sessions.');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchEnergyData = async () => {
      try {
        ...
        console.error(err);
        toast.error('Failed to fetch energy analytics for this session.');
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div className="sticky top-2 z-10 rounded-2xl border border-slate-300/70 bg-white/80 p-3 backdrop-blur dark:border-slate-700/70 dark:bg-[#0f141b]/80">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="rounded-3xl bg-slate-50 border border-slate-300 overflow-hidden dark:bg-[#141b25] dark:border-slate-700 relative group">
      ...
      {energyBuckets.length > 0 && (
        <div className="rounded-3xl bg-slate-50 border border-slate-300 p-6 dark:bg-[#141b25] dark:border-slate-700">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {energyBuckets.map((bucket, i) => (
              <div key={i} className="rounded-2xl border border-slate-300 bg-white p-6 dark:border-slate-700 dark:bg-[#0f141c]">
